<template>
  ...
  <div class="header">
    <v-app-bar height="84px" flat class="white">
      ...
      <!-- buttons field -->
      <v-spacer></v-spacer>
      <v-btn v-if="!subscribed" class="ma-12 subscribeBtn" rounded
             color="secondary"
             @click.stop="subscribeDialog = true">SUBSCRIBE TO NEWSLETTER</v-btn>
      <!--social media -->
      <v-btn icon
    ...
    <v-dialog v-model="subscribeDialog"
      ...
      <v-card id="subscribe-dialog">
        ...
        <v-flex class="pa-2 content" v-if="!subscribed">
          <div class="ma-3">
            <div class="heading title text-center ma-4 font-weight-bold orange--text">SUBSCRIBE FOR NEWSLETTER</div>
          ...
          <v-col cols="12" md="12">
            <v-form ref="form" v-model="valid" lazy-validation justify-center>
              <v-text-field v-model="subscriber.email"
                            :rules="emailRules"
                            label="E-mail"
                            required
                            outlined
              ...
              <v-select v-model="subscriber.select"
                        :items="newsletters"
                        :rules="[v => !!v || 'Item is required']"
                        label="GFO or OFM ?"
                        required
                        class="white--text"></v-select>
              ...
              <div class="text-center">
                <v-dialog v-model="dialog" max-width="400">
                  ...
                  <v-card>
                    ...
                    <v-card-text>
                      By subscribing to this website you agree to receive our newsletter every month.
                    </v-card-text>
                    <v-divider></v-divider>
                    <v-card-actions>
                      <v-spacer></v-spacer>
              ...
              <v-btn :disabled="!valid"
                     ...
                     block
                     large
                     @click="saveSubscriber">Subscribe</v-btn>
            </v-form>
          </v-col>
        </v-flex>
      </v-card>
